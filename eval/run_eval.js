...
import { retrieve } from "../backend/src/retrieval/hybrid.js";
...
async function retrievalPass(cfg) {
  ...
  for (const row of answerable) {
    ...
    const hits = row.expected_sections
      .map((want) => ranked.indexOf(want))
      .filter((i) => i !== -1);
...
async function answerPass(cfg) {
  const { answerStream } = await import("../backend/src/llm/answer.js");
  ...
  return {
    ...
    refusal_rate_out_of_scope: +(refusedOutOfScope / refusable.length).toFixed(
      3,
    ),
